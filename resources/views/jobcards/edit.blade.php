...
<div class="col-12">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="row g-3 mt-lg-3" style="">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <a class="btn btn-success f-right" href="{{ route('tasks.create', $job_card->id) }}">
                        <i class="me-1 fa fa-plus"></i> {{ __('global.newTask') }}
                    </a>
                    <a class="btn btn-default f-right mr-1" href="{{ route('tasks.excel', $job_card->id) }}">
            ...
            @if(count($tasks) == 0)
            <p class="txt-center mb-0">{{ __('global.noTask') }}</p>
            @else
            <table class="table table-custom mb-0 mt-lg-4 mydata-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ ++$i }}</td>
                        <td class="task-title-color" data-toggle="tooltip" data-bs-original-title="{{$task->title}}">{!! Str::of($task->title)->limit(10); !!}</td>
                        <td data-toggle="tooltip" data-bs-original-title="{{$task->description}}">{!! Str::of($task->description)->limit(20); !!}</td>
                        <td>
                            @if($task->status == 0)
                            <span class="badge bg-warning font-size-11">{{ $status[$task->status] }}</span>
                            @elseif($task->status == 1)
                            <span class="badge bg-danger font-size-11">{{ $status[$task->status] }}</span>
                            @elseif($task->status == 2)
                            <span class="badge bg-success font-size-11">{{ $status[$task->status] }}</span>
                            @elseif($task->status == 3)
                            <span class="badge bg-secondary font-size-11">{{ $status[$task->status] }}</span>
                            @else
                            <span class="badge bg-warning font-size-11">{{ $status[$task->status] }}</span>
                            @endif
                        </td>
                        <td>{!! Str::of($task->creator())->limit(30); !!}</td>
                        <td>
                            {!! Form::select('', $task->assign(), $task->assign(), array('class' => 'form-control','single')) !!}
                        </td>
                        <td>
                            ...
                            <a href="{{ route('tasks.edit', $task->id) }}" class="dropdown-item"><i class="me-3 fa fa-pencil"></i>{{ __('global.edit') }}</a>
                            <a href="{{ route('tasks.duplicate', $task->id) }}" class="dropdown-item"><i class="me-3 fa fa-copy"></i>{{ __('global.duplicate') }}</a>
                            @if(!empty(Auth::user()->getRoleNames()) && Auth::user()->hasExactRoles('SuperAdmin') || Auth::user()->hasExactRoles('Admin'))
                            <div class="dropdown-divider"></div>
                            <button class="dropdown-item btn-delete" data-taskID="{{ $task->id }}"><i class="me-3 fa fa-trash"></i>{{ __('global.delete') }}</button>
                            </div>
                            @endif
                        </td>
                    </tr>
