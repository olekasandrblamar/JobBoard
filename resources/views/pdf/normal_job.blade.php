...
<html>
    ...
    <body style="font-size: 11px !important;">
        ...
        <div style="border: solid 1px black; padding: 10px; margin-bottom: 20px;">
            <strong>{{ __('global.coordinater') }}</strong>: @if($job_card->coordinater != null) {{ $job_card->coordinater }} @else NONE @endif
            <strong>{{ __('global.university') }}</strong>: @if($job_card->university != null) {{ $job_card->university }} @else NONE @endif
        </div>
        ...
        @foreach($job_card->tasks as $key => $task)
            <h2 style="font-size: 11px !important;">
                {{ $task->title }}
                @if($task->status == 0)
            ...
            <div style="border: solid 1px red; padding: 10px; margin-bottom: 20px;">
                <div style="background-color: rgba(200, 200, 200, 0.8);">
                    <strong>{{ __('global.title') }}</strong>: {{ $comment->title }}
                    @if($comment->phase == null)
                    @else
