    ...
    <div class="container-fluid breadcrumb-style">
        <ol class="breadcrumb bg-transparent li_animate mb-3 mb-md-1">
            <li class="breadcrumb-item font-size-28">
                <a class="wp-title-color txt-deco" href="{{ route('jobcards.index') }}">{{ __('global.users') }}</a>
            </li>
        ...
        <div class="card-body">
            ...
            <table class="table table-bordered mt-3">
                <tr>
                    <th colspan="5">
                        {{ __('global.userList') }}
                        <a class="btn btn-warning float-end" href="{{ route('users.export') }}">{{ __('global.export') }}</a>
                    </th>
                </tr>
                <tr>
                    <th>{{ __('global.no') }}</th>
                    <th>{{ __('global.firstName') }}</th>
                    <th>{{ __('global.lastName') }}</th>
                    <th>{{ __('global.email') }}</th>
                    <th>{{ __('global.allowLogin') }}</th>
                </tr>
                ...
                <tr>
                    ...
                    <td>{{ $user->lastname }}</td>
                    <td>{{ $user->email }}</td>
                    <td>{{ $special[$user->allow_login] }}</td>
                </tr>
                @endforeach
            </table>
            @include('layouts.pagination.index', ['paginator' => $users])
        </div>
    </div>
@endsection
